...
export async function findAndSortAllVolsPerUniqueAddressesFromAll(): Promise<AddressVolume[]> {
  try {
    const uniqueAddresses = await findAndCountUniqueCallesPlusCalledContracts();
    const addressVolumes: AddressVolume[] = [];
...
export async function getTxIdsForPoolAndTargetAddress(poolAddress: string, targetAddress: string): Promise<number[]> {
  try {
...
const convertDateToUnixTime = (dateString: string) => {
