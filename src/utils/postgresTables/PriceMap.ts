...
async function filterCoinIds(coinIds: number[]): Promise<number[]> {
  const validCoinIds: number[] = [];
...
async function fetchAndStoreEarliestPriceTimestamp(): Promise<void> {
  try {
...
async function determineStartTime(firstPriceEntry: any): Promise<{ start: number; span: number }> {
...
async function handlePriceChartResponse(tokenAddress: string, coinId: number, priceChartResponse: any | "missing", latestTimestamp: number | null): Promise<void> {
  ...
  if (!priceChartResponse || !priceChartResponse.prices) return;
...
async function logTransactionCoinsStats(): Promise<void> {
  try {
