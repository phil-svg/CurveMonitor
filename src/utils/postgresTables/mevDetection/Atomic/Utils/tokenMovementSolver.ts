...
import { BalanceChange, CallTrace, CoinProperty, ITransactionTrace, TransferEvent } from "../../../../Interfaces.js";
...
export function getTokenBalanceChangesForUser(transferEvents: TransferEvent[], userAddress: string): BalanceChange[] {
...
export function calculateEthBalanceChangeForUser(callTrace: CallTrace, userAddress: string): number {
  let balanceChange = 0;
...
export function addEthBalanceChange(balanceChanges: any[], ethBalanceChange: number): any[] {
...
async function fetchAndSaveCoinProperty(tokenAddress: string, propertyName: CoinProperty, fetchFunction: (address: string) => Promise<any>): Promise<any> {
  if (tokenAddress === ETH_ADDRESS) return propertyName === "symbol" ? "ETH" : 18;
...
export async function adjustBalancesForDecimals(balanceChanges: BalanceChange[]): Promise<BalanceChange[]> {
...
export function getTransferEventsFromTraceForUser(callTraces: any[], userAddress: string): TransferEvent[] {
...
function handleTransferMethod(sender: string, receiver: string, amountHex: string, tokenAddress: string, tokenTransfers: TokenTransfer[]): void {
  const value = BigInt("0x" + amountHex);
  tokenTransfers.push({
    from: sender,
    to: receiver,
...
function handleUnwrapWethMethod(sender: string, receiver: string, amountHex: string, tokenTransfers: TokenTransfer[]): void {
  const value = BigInt("0x" + amountHex);
...
function handleWrapEthMethod(sender: string, receiver: string, amountHex: string, tokenTransfers: TokenTransfer[]): void {
  const value = BigInt("0x" + amountHex);
...
function handleMintMethod(input: string, tokenAddress: string, tokenTransfers: TokenTransfer[]): void {
...
function extractTransferDetails(input: string, from: string): { sender: string; receiver: string; amountHex: string } {
  let sender = "";
  let receiver = "";
  let amountHex = "";
...
function extractTokenTransfers(trace: ITransactionTrace, tokenTransfers: TokenTransfer[]): void {
...
export async function getFullBalanceChangesForAddress(transactionTrace: ITransactionTrace[], user: string): Promise<BalanceChange[] | null> {
...
type SwapPair = [ExtendedReadableTokenTransfer, ExtendedReadableTokenTransfer];
type EtherWrapUnwrapPair = [ReadableTokenTransfer, ReadableTokenTransfer];
...
function identifyMintPairs(transfers: ReadableTokenTransfer[]): { mintPairs: SwapPair[]; remainingTransfers: ReadableTokenTransfer[] } {
  const mintPairs: SwapPair[] = [];
...
type LiquidityEvent = [ReadableTokenTransfer, ReadableTokenTransfer[]];
