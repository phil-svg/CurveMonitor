...
export async function getBlockBuilderFromFromAndTo(txId: number, from: string, to: string): Promise<string | null> {
...
function getBribeInEth(transfers: ReadableTokenTransfer[]): number | null {
...
export async function getBribeInUSDfromTx(txId: number): Promise<number | null> {
  const fullCleanedTransfers = await getCleanedTransfersForTxIdFromTable(txId);
  if (!fullCleanedTransfers) return null;
  ...
  if (!ethPriceAtThatTime) return null;

  return bribeInEth * ethPriceAtThatTime;
}

export async function getGasSpendingsInUSDfromTx(txId: number): Promise<number | null> {
...
export async function solveRevenueLowerBoundInUSD(txId: number): Promise<number> {
  let gasMoney = await getGasSpendingsInUSDfromTx(txId);
  if (!gasMoney) gasMoney = 0;

  let bribeMoney = await getBribeInUSDfromTx(txId);
  if (!bribeMoney) bribeMoney = 0;
