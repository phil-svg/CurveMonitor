...
export async function wasPoolCalledDirectly(txId: number, to: string): Promise<boolean> {
...
export async function isCexDexArbCandidate(txId: number, numOfTransfers: number): Promise<boolean> {
  ...
  if (hasIllegalOutbound(from, to, cleanedTransfers)) return false;
...
export async function isCexDexArb(txId: number): Promise<boolean | "unable to fetch"> {
  ...
  if (await wasPoolCalledDirectly(txId, to)) return false;

  if (!(await isCexDexArbCandidate(txId, numOfTransfers))) return false;

  if (await isActuallyBackrun(txId)) return false;

  if (await readAbiFromAbisEthereumTable(to)) return false;
