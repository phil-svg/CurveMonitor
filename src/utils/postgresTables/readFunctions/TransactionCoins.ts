...
export async function countEntriesWithNullDollarValue(): Promise<number> {
...
export async function countNullDollarValueForCoin(coin_id: number): Promise<number> {
  const count = await TransactionCoins.count({
    where: {
      coin_id: coin_id,
      dollar_value: null,
...
export async function getTotalEntriesForCoin(coin_id: number): Promise<number> {
  const totalEntries = await TransactionCoins.count({
    where: {
      coin_id: coin_id,
    },
...
export async function getAllUniqueSwapCounterPartCoinIds(coinId: number): Promise<number[]> {
  // Find all tx_ids that have both 'in' and 'out' directions
  const transactionIdsWithBothDirections = (
    await TransactionCoins.sequelize!.query(
      `
  ...
  ).map((transaction: any) => transaction.tx_id);
  ...
  const swapCoinIds = new Set<number>();
...
export async function findAndModifySwapTransactions(coinId1: number, coinId2: number): Promise<void> {
  ...
  for (let tx_id of transactionIds) {
    ...
    if (transactionCoins.length !== 2) continue;
    ...
    if (!entryCoin1 || !entryCoin1.dollar_value) continue;
...
export async function findSwapsForCoin(coinId: number): Promise<number[]> {
...
export async function findSwapTransactionsForTwoCoins(coinId1: number, coinId2: number): Promise<number[]> {
...
export async function findTransactionIdsForCoinWithNullDollarValue(coinId: number): Promise<number[]> {
...
export async function getPriceArrayFromSwapEntries(coinId: number): Promise<{ unixtime: number; price: number }[]> {
