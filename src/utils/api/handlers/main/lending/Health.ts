...
import {
  checkMarketsForLendingUser,
  getRealTimeHealthDataForAllMarketsOfUser,
  getRecentHealthDataForAllMarketsOfUser,
} from './Utils.js';

export const handleUserLendingHealth = (socket: Socket) => {
  let intervalId: string | number | NodeJS.Timeout | null | undefined = null; // Track the interval ID for cleanup

  const emitHealthData = async (userAddress: string) => {
    try {
  ...
  const setupDisconnectHandler = (userAddress: string) => {
  ...
  socket.on('subscribeToUserHealthLendingStream', async (userAddress: string) => {
    const marketsForLendingUser = await checkMarketsForLendingUser('ethereum', userAddress, 1, 50);
    ...
    intervalId = setInterval(async () => {
      ...
      if (realTimeData) socket.emit('RealTimeHealthUpdate', realTimeData);
    }, 12000);
