...
async function findTop20AddressesByVolume(volumeForAddress: Record<string, number>): Promise<[string, number][]> {
  return Object.entries(volumeForAddress)
    .sort(([, volumeA], [, volumeB]) => volumeB - volumeA)
    .slice(0, 20);
}

export async function generateTopToVolAddresses(startDate: string, endDate: string) {
...
async function calculateTotalVolumeForPools(
  ...
  for (const [poolAddress, txIds] of Object.entries(groupedTransactions)) {
    ...
    for (const txId of txIds) {
      const volume = await priceTransactionFromTxId(txId);
      totalVolume += Number(volume?.toFixed(0)) || 0;
    }
...
function rankPoolsByVolume(volumeForPool: Record<string, number>): [string, number][] {
...
export async function calculateTotalVolumeForTransactionsInDb(): Promise<number> {
  let totalVolume = 0;
  let offset = 0;
...
export async function getSwapVolumeForPoolAndToAddressForEachSwapDirection(
  poolAddress: string,
  toAddress: string,
  startDate: string,
  endDate: string
): Promise<{ [swapDirection: string]: { count: number; totalVolume: number } }> {
  const startUnixTime = new Date(startDate).getTime() / 1000;
  const endUnixTime = new Date(endDate).getTime() / 1000;
...
export async function generateTopFromVolAddressesForSpecificToAddress(
  ...
  const volumeByFromAddress: Record<string, number> = {};
...
export async function getSwapVolumeBucketsForPoolAndToAddress(
  ...
  const swapBuckets: { [bucket: string]: { count: number; totalVolume: number } } = {};

  transactions.forEach((transaction) => {
    transaction.transactionCoins.forEach((txCoin) => {
      if (!txCoin.dollar_value) return;
      const dollarVolume = parseFloat(txCoin.dollar_value.toString());

      if (dollarVolume < 100000) console.log(transaction.tx_hash, dollarVolume);
