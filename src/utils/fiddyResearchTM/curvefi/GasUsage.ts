...
export async function getGasUsedArrayForPoolAndFunction(
  poolAddress: string,
  functionName: string,
  txHash: string
): Promise<number[]> {
...
async function getAllTxHashesForPoolAndTimeWindow(
  poolAddress: string,
  startDate: string,
  endDate: string
): Promise<string[] | null> {
...
function sortGasUsageBrackets(
  ...
  Object.keys(gasUsageBrackets).forEach((functionName) => {
    ...
    sortedGasUsageBrackets[functionName] = sortedBracketsArray.reduce(
      (acc, [key, value]) => {
        acc[key] = value;
        return acc;
      },
...
async function getGasUsageBrackets(
  stableswapPoolAddressArr: string[],
  startDate: string,
  endDate: string
    ...
    for (const functionName of relevantFunctionNames) {
      ...
      for (const txHash of allTxHashes) {
        ...
        if (localTxCounter % 100 === 0) {
          console.log(
            'Pool: ' + poolCounter + '/' + poolChecks,
            'Pool Progress:',
            (100 * (localTxCounter / (allTxHashes.length * relevantFunctionNames.length))).toFixed(0) + '%'
          );
        }
  ...
  console.log(
    ...
    'pools,',
    txHashesChecks,
    'txHashes, and comepleted',
    gasUsageChecks,
    'gasUsagesChecks, numResults:',
    numOfCounts
  );
...
async function getGasUsageBracketsForSpecificConditions(
  stableswapPoolAddressArr: string[],
  startDate: string,
  endDate: string
): Promise<Record<string, Record<string, number>>> {
