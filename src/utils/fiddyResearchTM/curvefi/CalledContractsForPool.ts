...
export async function poolTrafficCalledContractsWOdb() {
  ...
  const volumeByAddress: { [key: string]: number } = {};
  ...
  if (events && Array.isArray(events)) {
    for (const [index, event] of events.entries()) {
      try {
        ...
        if (index % 10 === 0) {
          console.log(
            `Processing event ${index + 1} of ${events.length} (${(((index + 1) / events.length) * 100).toFixed(1)}%)`
          );
        }
        ...
        const to = tx.to;
        const { sold_id, tokens_sold, tokens_bought } = (event as any).returnValues;
