...
export async function aggregateCexDexBotVolumeOverTime(botAddress: string, poolAddress: string): Promise<WeeklyVolume[]> {
  ...
  const weeklyVolumesMap = new Map<string, WeeklyVolume>();
  ...
  for (const entry of cexDexArbsEntries) {
    ...
    const currentVolume = weeklyVolumesMap.get(weekStartStr)?.volume || 0;
...
export async function writeDBotVolOverTimeataToExcel(weeklyVolumes: WeeklyVolume[]): Promise<void> {
