...
import {
  getBlockTimeStampFromProvider,
  getPastEvents,
  getTxFromTxHashAndProvider,
} from '../../../../../web3Calls/generic.js';
import { getCustomContract } from './CustomContract.js';
import { ChainName, getCleanedTransfersWOdbWOtraceForChain } from './CleanTransfersWOdb.js';
import { isCexDexArbWOdb } from './CexDexArbs.js';
import { getFuzzyVolumeForGenericTx } from './Volume.js';
import {
  VolumeDictionary,
  saveResultsToJsonFile,
  saveVolumeDictionaryToJsonFile,
  writeTransactionsToCSV,
} from './Helper.js';
import {
  aggregateAndSaveVolumesByToAddress,
  filterAndSaveTransactionsByToAddress,
  loadAndCalculateHourlyVolumes,
  saveHourlyDataToCsvForVisuals,
} from './Visuals.js';
...
async function solveSingleTxHash(
  txHash: string,
  chain: ChainName,
  ...
  const cleanedTransfers = await getCleanedTransfersWOdbWOtraceForChain(
    txHash,
    chain,
    web3HttpProvider,
    to,
  ...
  let cexDexArbInfo: boolean | 'unable to fetch';
  ...
  if (knownBotsSet.has(to.toLowerCase())) {
    ...
    cexDexArbInfo = await isCexDexArbWOdb(
      cleanedTransfers,
      from,
      to,
      blockNumber,
...
export async function anyPoolMevCheck() {
  ...
  filterAndSaveTransactionsByToAddress(
    './result.json',
    './0x758d00d2235bcb30861421b84a3656f29c7d09c1.csv',
    '0x758d00d2235bcb30861421b84a3656f29c7d09c1'
  );
  ...
  const uniqueTxHashesSet = new Set<string>();
  ...
  let fromVolumes: VolumeDictionary = {};
  let toVolumes: VolumeDictionary = {};

  for (const txHash of uniqueTxHashesSet) {
    if (txHash !== '0xd2ac62318cb190669dfdadf7a97dd0c78f478dd608b1a5e8b8c7d6a79da3931f') continue;
    // console.log('txHash:', txHash);

    uniqueTxHashesSet.add(txHash);
    const transactionAnalysisResult = await solveSingleTxHash(txHash, chain, web3HttpProvider, poolContractAddress);
    if (!transactionAnalysisResult) continue;
    ...
    */
    ...
    if (transactionAnalysisResult?.isCexDexArb === true) counterCexDexArbs++;
    counter++;
    if (counter % 50 === 0) {
