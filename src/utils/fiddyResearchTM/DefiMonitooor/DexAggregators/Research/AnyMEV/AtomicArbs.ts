...
import {
  botOrFromTransferToLeaf,
  filterTransfersByAddress,
  getBalanceChangeForAddressFromTransfers,
  hasAtLeastTwoPairs,
  hasEnoughSwaps,
  hasExternalTokenInflow,
  hasMissmatchingForOriginLeafesToTo,
  highestBlockPositionWorthChecking,
  is1InchInvolvedInTransfers,
  isAtomicArbCaseValueStaysWithFromOrTo,
  removeFalseDupes,
  toHasInflowFromLeafOtherThanFrom,
  wasDsProxy,
...
async function checkCaseValueStaysWithFromOrToWOdb(
  ...
  const dummyArbitrageResult: FormattedArbitrageResult = {
    extractedValue: [
      {
...
async function isAtomicArbCaseValueGoesOutsideFromOrToWOdb(
...
async function checkCaseValueGoesOutsideFromOrToWOdb(
  onlyToTransfers: ReadableTokenTransfer[],
  cleanedTransfers: ReadableTokenTransfer[],
  txHash: string,
  transactionDetails: TransactionDetails,
  ...
  const [isAtomicArb, value] = await isAtomicArbCaseValueGoesOutsideFromOrToWOdb(
    onlyToTransfers,
    cleanedTransfers,
    balanceChangeFrom,
    balanceChangeTo,
  ...
  if (isAtomicArb) return 'isAtomicArb';
  return 'isNot';
}

async function isMakerProxyForChain(contractAddress: string, chain: ChainName): Promise<boolean> {
...
export async function isGlobalBackrunForChainFuzzyWOdb(
  blockNumber: number,
  txPosition: number,
  from: string,
  web3HttpProvider: any
): Promise<boolean | null> {
  if (txPosition <= 1) return false;
  ...
  if (previousTx.to.toLowerCase() !== from.toLowerCase()) return false;
...
export async function solveAtomicArbWOdbForChain(
  txHash: string,
  cleanedTransfers: ReadableTokenTransfer[],
  from: string,
  to: string,
  ...
  chain: ChainName,
  web3HttpProvider: any
): Promise<TransactionDetailsForAtomicArbs | null | 'not an arb' | 'arb'> {
  ...
  // checking case 1
  let formattedArbitrage = await checkCaseValueStaysWithFromOrToWOdb(
    cleanedTransfers,
    txHash,
    txDetails,
    balanceChangeFrom,
  ...
  if (formattedArbitrage) {
    ...
    isArb = await checkCaseValueGoesOutsideFromOrToWOdb(
      onlyToTransfers,
      cleanedTransfers,
      txHash,
      txDetails,
