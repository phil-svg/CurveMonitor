...
export async function getFuzzyVolumeForGenericTx(
  ...
  const isSwap =
    cleanedTransfers.some((transfer) => transfer.to.toLowerCase() === contractAddress) &&
  ...
  for (const tranfer of cleanedTransfers) {
    ...
    if (from !== contractAddress && to !== contractAddress) continue;
