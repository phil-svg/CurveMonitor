import fs from 'fs';
import {
  getTokenBalanceForWalletAndTokenAndBlockFromChain,
  getTokenDecimalsFromChain,
} from '../../../../helperFunctions/Web3.js';
...
function convertIntervalToBlockGap(interval: Interval): number {
...
async function getBtcPriceFromTricrypto2ForBlockFromChain(blockNumber: number): Promise<number> {
  const contract = await getTricrypto2Contract();
  const btcPrice = (await web3Call(contract, 'get_dy', [1, 0, 100000000], blockNumber)) / 1e6;
  return btcPrice;
}

async function fetchTokenBalanceForWalletForTimeAndInterval(
  tokenAddress: string,
  decimals: number,
  ...
  while (currentUnixtime <= endUnixtime) {
    ...
    const balance = await getTokenBalanceForWalletAndTokenAndBlockFromChain(
      tokenAddress,
      decimals,
      walletAddress,
      blockNumber
...
export async function studyTokenBalanceOfWallet() {
  ...
  const interval: Interval = { value: 30, unit: 'minutes' };

  await fetchTokenBalanceForWalletForTimeAndInterval(
    tokenAddress,
    decimals!,
    walletAddress,
    startUnixtime,
