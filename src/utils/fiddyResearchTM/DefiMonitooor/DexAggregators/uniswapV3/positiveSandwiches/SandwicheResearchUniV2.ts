...
import { SimplifiedUniV3SwapEvent, SwapEventUniV2, SwapEventUniV3 } from './Utils.js';
import fs from 'fs';

function reverseSwapDirection(swapDirection: string): string {
...
function findSandwiches(events: SimplifiedUniV3SwapEvent[]) {
  // Group events by blockNumber
  const groupedByBlock: { [blockNumber: number]: SimplifiedUniV3SwapEvent[] } = {};
  ...
  const sandwiches: {
    blockNumber: number;
    sequence: SimplifiedUniV3SwapEvent[];
  }[] = [];
  ...
  Object.keys(groupedByBlock).forEach((blockNumberStr) => {
    ...
    for (let i = 0; i < N; i++) {
      ...
      let victimTxs: SimplifiedUniV3SwapEvent[] = [];
      let backrun: SimplifiedUniV3SwapEvent | null = null;
...
function formatEvents(events: SwapEventUniV2[], poolInfo: UniswapV2PoolInfo): SimplifiedUniV3SwapEvent[] {
  return events.map((event) => {
    ...
    const swapDirection =
      soldToken === poolInfo.token1.name
...
async function getSwapEvents(allEvents: any[]): Promise<SwapEventUniV2[]> {
...
async function getEvents(poolAddress: string, blockNumber?: number): Promise<SwapEventUniV3[]> {
...
async function getExtendedGasInfo(blockNumber: number, position: number): Promise<string | null> {
  try {
...
function filterProbSammichWithAtLeast1MatchingTx(
  sandwiches: { blockNumber: number; sequence: SimplifiedUniV3SwapEvent[] }[],
  formattedEvents: SimplifiedUniV3SwapEvent[]
): {
  blockNumber: number;
  victimTxHash: string;
  otherTxHashes: string[];
}[] {
  return sandwiches
    .map((sandwich) => {
      ...
      if (sequence.length < 3) return null; // Need at least 3 transactions

      const frontrun = sequence[0];
      const backrun = sequence[sequence.length - 1];
      const centerTxs = sequence.slice(1, -1);

      if (centerTxs.length === 0) return null; // No center transactions (victims)
      ...
      if (sameDirectionCount >= 4) {
        const otherTxHashes = formattedEventsForBlock
          .filter((event) => {
            ...
            return (
              txSwapDirection === victimSwapDirection &&
...
async function addGasInfoToSandwiches(
...
> {
  ...
  for (let i = 0; i < probSammichWithAtLeast1MatchingTx.length; i += concurrencyLimit) {
    ...
    const chunkResults = await Promise.all(
      chunk.map(async (sandwich) => {
        // Find the victim transaction in formattedEvents to get the correct position
        const victimEvent = formattedEvents.find(
          (event) => event.txHash === sandwich.victimTxHash && event.blockNumber === sandwich.blockNumber
        );
        ...
        for (let j = 0; j < sandwich.otherTxHashes.length; j += concurrencyLimit) {
          ...
          const txChunkResults = await Promise.all(
            txHashChunk.map(async (txHash) => {
              // Find the event for the other transaction to get the correct position
              const otherEvent = formattedEvents.find(
                (event) => event.txHash === txHash && event.blockNumber === sandwich.blockNumber
              );
...
export function filterSandwichesByPositionAndGas(
  probSammichWithAtLeast1MatchingTxAndGas: {
    blockNumber: number;
    victim: {
      txHash: string;
      gasPrice: string | null;
...
async function getPoolBalanceAtBlock(
  poolAddress: string,
  poolInfo: UniswapV2PoolInfo,
  blockNumber: number
...
interface UniswapV2PoolInfo {
  token0: TokenInfo;
  token1: TokenInfo;
}

async function loadUniswapV2Pool(poolAddress: string): Promise<UniswapV2PoolInfo> {
...
interface XYKSwapResult {
  outputAmount: number;
  newToken0Reserve: number;
  newToken1Reserve: number;
}

function calculateXYKSwap(inputAmount: number, inputReserve: number, outputReserve: number): XYKSwapResult {
...
async function addMissingTxHashes(
  filteredResults: {
    blockNumber: number;
    victim: {
      txHash: string;
...
> {
  return Promise.all(
    filteredResults.map(async (result) => {
      ...
      const missingTransactions = formattedEvents.filter(
        (event) => event.blockNumber === blockNumberInt && !existingTxHashes.has(event.txHash) // Exclude already included transactions
      );

      // Fetch gas info for all missing transactions
      const missingTxWithGasInfo = await Promise.all(
        missingTransactions.map(async (missingEvent) => {
...
function handleVictimTrace(victimTrace: any[]) {
...
async function processVictimTransaction(
  result: {
    blockNumber: number;
    victim: {
      txHash: string;
  ...
  if (victimPosition === null || victimPosition === undefined) return;
  ...
  const swapsWithoutBot = result.others.filter(
    (other) =>
...
interface LabeledSwapEvent extends SimplifiedUniV3SwapEvent {
  label?: 'frontrun' | 'center' | 'backrun';
}

interface Sandwich {
  blockNumber: number;
  frontrun: LabeledSwapEvent;
  center: LabeledSwapEvent | LabeledSwapEvent[];
  backrun: LabeledSwapEvent;
}

function extractSandwiches(
  ...
  for (const group of groupsWithDoubledUser) {
    ...
    const userPositions: { [user: string]: SimplifiedUniV3SwapEvent[] } = {};
    ...
    for (const [user, events] of Object.entries(userPositions)) {
      if (events.length < 2) continue; // We need at least two transactions for a sandwich

      // Sort by position to identify frontrun and backrun
      const sortedEvents = events.sort((a, b) => a.position - b.position);
      const frontrun: LabeledSwapEvent = { ...sortedEvents[0], label: 'frontrun' };
      const backrun: LabeledSwapEvent = { ...sortedEvents[sortedEvents.length - 1], label: 'backrun' };
      ...
      const center = sequence
        ...
        .map((event) => ({ ...event, label: 'center' })) as LabeledSwapEvent[];
...
async function addGasInfoToSandwichesFull(
  sandwiches: {
    blockNumber: number;
    frontrun: SimplifiedUniV3SwapEvent;
    center: SimplifiedUniV3SwapEvent | SimplifiedUniV3SwapEvent[];
    backrun: SimplifiedUniV3SwapEvent;
  }[],
  formattedEvents: SimplifiedUniV3SwapEvent[]
): Promise<Sandwich[]> {
  const concurrencyLimit = 10;
  ...
  for (let i = 0; i < sandwiches.length; i += concurrencyLimit) {
    ...
    const chunkResults = await Promise.all(
      chunk.map(async (sandwich) => {
        ...
        const frontrunEvent = formattedEvents.find(
          (event) => event.txHash === sandwich.frontrun.txHash && event.blockNumber === sandwich.blockNumber
        );
        const frontrunGasInfo = frontrunEvent
          ? await getExtendedGasInfoFull(frontrunEvent.blockNumber, frontrunEvent.position)
          : null;

        const frontrun: LabeledSwapEvent = {
          ...sandwich.frontrun,
          label: 'frontrun',
          gasInfo: frontrunGasInfo,
        };

        // Get gas info for the backrun transaction
        const backrunEvent = formattedEvents.find(
          (event) => event.txHash === sandwich.backrun.txHash && event.blockNumber === sandwich.blockNumber
        );
        const backrunGasInfo = backrunEvent
          ? await getExtendedGasInfoFull(backrunEvent.blockNumber, backrunEvent.position)
          : null;

        const backrun: LabeledSwapEvent = {
          ...sandwich.backrun,
          label: 'backrun',
          gasInfo: backrunGasInfo,
        };

        // Get gas info for all center transactions
        const centerEvents = Array.isArray(sandwich.center) ? sandwich.center : [sandwich.center];
        const centerWithGasInfo = await Promise.all(
          centerEvents.map(async (centerEvent) => {
            const event = formattedEvents.find(
              (e) => e.txHash === centerEvent.txHash && e.blockNumber === sandwich.blockNumber
            );
...
async function getExtendedGasInfoFull(blockNumber: number, position: number): Promise<ExtendedGasInfo | null> {
  try {
    const transaction = await getBlockTransactionDetails(blockNumber, position);
    return {
...
interface PoolData {
  poolAddress: string;
  poolInfo: UniswapV2PoolInfo;
  sandwichData: LabeledSwapEvent[][];
}

interface VictimSim {
  victimTxHash: string;
  actualOutputAmount: number;
  victimSimAmountOut: number;
  amountOutMin: number;
}

interface SimDataInterface {
  poolAddress: string;
  poolInfo: UniswapV2PoolInfo;
  sandwichOverview: SandwichOverview;
  victimTransactions: VictimSim[];
}

interface SandwichOverview {
  numberSwaps: number;
  numberSandwiches: number;
  numberSandwichGotVictimTxMined: number;
}

async function filterFullSetByAllowedAddresses(
...
async function fetchCallData(txHash: string) {
  try {
...
async function processTransactionsAndSave() {
  ...
  for (let i = 0; i < filteredData.length; i++) {
    ...
    const amountOutMinParam = decodedData.params.find((param: any) => param.name === 'amountOutMin');
    ...
    const matchingSandwich = sandwichData.find((poolEntry: any) =>
      poolEntry.sandwichData.some((sandwich: any) => sandwich.some((tx: any) => tx.txHash === victimTxHash))
    );
    ...
    const sandwichEntry = matchingSandwich.sandwichData.find((sandwich: any) =>
      sandwich.some((tx: any) => tx.txHash === victimTxHash)
    );
    const victimTx = sandwichEntry.find((tx: any) => tx.txHash === victimTxHash);
...
const filteredTxHashes = new Set(filteredData.map((entry: any) => entry.victimTxHash));
...
async function addMinAmountOutToCenter() {
    ...
    for (let sandwich of poolEntry.sandwichData) {
      for (let tx of sandwich) {
        if (filteredTxHashes.has(tx.txHash) && tx.label === 'center') {
          const entry = filteredData.find((e: any) => e.victimTxHash === tx.txHash);
  ...
  fs.writeFileSync(
    'filteredSandwichDataUniswapV2_withMinAmountOut.json',
    JSON.stringify(sandwichData, null, 2),
    'utf-8'
  );
...
function filterSandwichesWithMinAmountOut() {
  ...
  const filteredData = data
    .map((poolEntry: any) => ({
      ...poolEntry,
      sandwichData: poolEntry.sandwichData.filter((sandwich: any) =>
        sandwich.some((tx: any) => tx.label === 'center' && tx.minAmountOut !== undefined)
      ),
    }))
    .filter((poolEntry: any) => poolEntry.sandwichData.length > 0);
...
async function getMinAmountOut(tx: any, poolInfo: any) {
  const input = await fetchCallData(tx.txHash);
  if (!input) return 'unknown';

  const decodedData = abiDecoder.decodeMethod(input);
  const amountOutMinParam = decodedData?.params.find((param: any) => param.name === 'amountOutMin');
  if (!amountOutMinParam) return 'unknown';
...
const simData: SimDataInterface[] = [];
let victimTransactions: VictimSim[] = [];
let sandwichOverview: SandwichOverview;
...
function removeDuplicateVictimTxHash() {
  ...
  const deduplicatedData = data.filter((entry: { victimTxHash: string }) => {
...
function filterFalsePositiveSandwiches() {
  ...
  function fuzzyMatch(amount1: number, amount2: number): boolean {
  ...
  const filteredData = data.map((entry: any) => ({
    ...entry,
    sandwichData: entry.sandwichData.filter((sandwich: any[]) => {
      const frontrun = sandwich.find((tx: any) => tx.label === 'frontrun');
      const backrun = sandwich.find((tx: any) => tx.label === 'backrun');
...
function filterVictimTransactions() {
  ...
  minAmountsData.forEach((entry: any) => {
    entry.sandwichData.forEach((sandwich: any[]) => {
      sandwich.forEach((transaction: any) => {
        if (transaction.txHash) validTxHashes.add(transaction.txHash);
      });
    });
  });
  ...
  const filteredSimData = simData
    .map((entry: any) => ({
      ...entry,
      victimTransactions: entry.victimTransactions.filter((victimTx: any) => validTxHashes.has(victimTx.victimTxHash)),
    }))
    .filter((entry: any) => entry.victimTransactions.length > 0);
      ...
      for (let i = 0; i < sandwich.length; i++) {
        ...
        if (tx.label === 'center' && tx.minAmountOut === undefined) {
          ...
          if (minAmountOut !== 'unknown') {
            ...
            poolEntry.sandwichData = poolEntry.sandwichData.filter((s: any) => s !== sandwich);
