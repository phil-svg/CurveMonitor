...
import { getInchv5Abi, getInchv6Abi } from '../ABIs.js';
...
async function fetchTransactionsForPoolAndToAddressAndTimeDuration(
  poolAddress: string,
  toAddress: string,
  startDate: string,
  endDate: string
...
export async function whatWasSwappedAnalytics(
  poolAddress: string,
  toAddress: string,
  startDate: string,
  endDate: string
) {
  const transactions = await fetchTransactionsForPoolAndToAddressAndTimeDuration(
    poolAddress,
    toAddress,
    startDate,
    endDate
  ...
  let swapVolumes: { [direction: string]: { volume: number; count: number } } = {};
  ...
  for (const transaction of transactions) {
    ...
    if (balanceChanges.length === 2) {
      ...
      if (!price) continue; // Skip if neither price is available
    ...
    try {
      ...
      try {
        if (error instanceof Error && 'recommendedBlockRange' in error) {
          const recommendedBlockRange = (error as any).recommendedBlockRange;
          if (
            ...
            for (
              let fromBlock = recommendedBlockRange.start;
              fromBlock < recommendedBlockRange.end;
              fromBlock += 5000
            ) {
...
async function getJsonFor1Inch() {
  ...
  const txHashesAndBlockNumbers: {
    txHash: string;
    blockNumber: string;
  }[] = ethersanJSON.map((row) => ({
    txHash: row.Txhash,
...
interface CsvRow {
  ...
  'Value_IN(ETH)': string;
  'Value_OUT(ETH)': string;
  'CurrentValue @ $3457.15564411958/Eth': string;
  'TxnFee(ETH)': string;
  'TxnFee(USD)': string;
  'Historical $Price/Eth': string;
  Status: string;
  ErrCode: string;
  Method: string;
}

async function getJsonParsedEtherscanCsv(csvFilePath: string): Promise<CsvRow[]> {
  const fileContent = fs.readFileSync(csvFilePath, { encoding: 'utf-8' });

  return new Promise((resolve, reject) => {
    parse(fileContent, { columns: true }, (err, rows: CsvRow[]) => {
      if (err) {
        reject(err);
      } else {
        resolve(rows);
...
function formatSwaps(
  ...
  userAmountSold: number;
  boughtTokenSymbol: string;
  userAmountReceived: number;
  swappedAt3Pool: number;
  swappedAtUniswapV3: number;
}[] {
  const _3PoolAddress = '0xbebc44782c7db0a1a60cb6fe97d0b483032ff1c7';
  const uniV3UsdcUsdtPoolAddress = '0x3416cf6c708da44db2624d63ea0aaef7113527c6';
...
function filterSwaps(
  ...
  _3PoolAddress: string,
  uniV3UsdcUsdtPoolAddress: string
) {
  const filteredSwaps = [];
...
function getDirectSwaps(cleanedTransfers: any[]) {
  const allowedSwaps = [
    { from: 'USDC', to: 'USDT' },
    { from: 'USDT', to: 'USDC' },
    { from: 'USDC', to: 'DAI' },
  ...
  for (let i = 0; i < cleanedTransfers.length - 1; i++) {
    ...
    if (
      allowedSwaps.some(
        (swap) =>
          transfer1.tokenSymbol === swap.from &&
