...
async function getAllTxForSameBlockAndPoolWithoutTheSandwich(txId: number, poolId: number, excludedTxIds: number[]): Promise<Transactions[]> {
...
function checkMatchingSymbolsAndDirections(simplifiedTxData: any, simplifiedSandwichTxData: any): boolean {
  ...
  const allMatchesFound = simplifiedTxData.every((txDataCoin: { symbol: any; direction: any }) => {
    return simplifiedSandwichTxData.some(
      (sandwichDataCoin: { symbol: any; direction: any }) => txDataCoin.symbol === sandwichDataCoin.symbol && txDataCoin.direction === sandwichDataCoin.direction
    );
  });
...
function findEntryByDirection(entries: SimplifiedSandwichTxData[], direction: "in" | "out"): SimplifiedSandwichTxData | undefined {
