...
import {
  ...
  calculateDailyAtomicArbVolumes,
...
function aggregateVolumes(aggregate: { [day: string]: number }, dailyVolumes: { [day: string]: number }): void {
...
type WeeklyVolumes = Record<string, number>;

function aggregateToWeeklyVolumes(dailyVolumes: DailyVolumes): WeeklyVolumes {
...
export async function calculateAndSaveAggregateWeeklyVolumeReport(startUnixtime: number, endUnixtime: number) {
  const poolIds = await getAllPoolIds();
  ...
  const reformattedData: Record<string, number[]> = {};
