import {
  ...
  findCoinSymbolById,
  getCoinIdByAddress,
} from '../../utils/postgresTables/readFunctions/Coins.js';
import {
  getEthPriceWithTimestampFromTable,
  getPriceFromDb,
} from '../../utils/postgresTables/readFunctions/PriceMap.js';
...
import { MarketData } from './CrvUSD.js';
import {
  CurveTricryptoOptimizedWETH,
  getCrvUSD_USDT,
  getLlammaContract,
  getTricrypto2Contract,
  getTricryptoUSDCContract,
  getTricryptoUSDTContract,
} from './Contracts.js';
...
async function getCallTraceViaRpcProvider(txHash: string): Promise<any> {
...
function getTokenBalanceChangesOld(transferEvents: TransferEvent[], userAddress: string): BalanceChange[] {
...
function calculateEthBalanceChangeFilteredForLlama(callTrace: any[], userAddress: string, market: MarketData): number {
  ...
  for (const call of callTrace) {
    ...
    if (call.action.callType !== 'call') continue;

    // Parse the value (default to 0 if value is missing or invalid)
    const value = parseInt(call.action.value ?? '0', 16);

    // Filter for calls involving the llamma address
    const isLlammaInvolved =
      call.action.from?.toLowerCase() === llammaAddress || call.action.to?.toLowerCase() === llammaAddress;

    if (!isLlammaInvolved) continue;
...
export async function getPrice(unixtime: number, address: string): Promise<number | null> {
  const price = await getHistoricalTokenPriceFromDefiLlama(address, unixtime);
  return price;
}

async function calculateAbsDollarBalance(unixtime: number, decimalAdjustedBalanceChanges: any[]): Promise<number> {
  let total = 0;

  for (const item of decimalAdjustedBalanceChanges) {
    // Convert WETH address to ETH address
    const tokenAddress =
      item.token.toLowerCase() === '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2' ? ADDRESS_ETH : item.token;

    let price = await getPriceFromDb(tokenAddress, unixtime);
    if (!price) price = await getPrice(unixtime, tokenAddress);
...
async function getPriceOnchain(tokenAddress: string, blockNumber: number): Promise<number | null> {
...
async function getDyOnchain(tokenAddress: string, blockNumber: number, dx: string): Promise<number | null> {
  if (tokenAddress.toLowerCase() === ADDRESS_CRVUSD.toLowerCase()) {
    const contract = await getCrvUSD_USDT();
...
async function calculateAbsDollarBalanceOnchain(
  unixtime: number,
  decimalAdjustedBalanceChanges: any[],
  blockNumber: number,
  userTransfersInAndOut: TransferEvent[]
): Promise<number> {
  let total = 0;
...
function filterForLlama(userTransfersInAndOut: TransferEvent[], market: MarketData): TransferEvent[] {
  const llammaAddress = market.llamma.toLowerCase();
...
async function getRevenueForAddress(
  unixtime: number,
  CALL_TRACE: any,
  user: string,
  market: MarketData,
  ...
  if (print) console.log(user, 'userTransfersInAndOut', userTransfersInAndOut);

  const filteredForLlama = filterForLlama(userTransfersInAndOut, market);
  if (print) console.log(user, 'filteredForLlama', filteredForLlama);

  const balanceChanges = getTokenBalanceChanges(filteredForLlama, user);
  if (print) console.log(user, 'balanceChanges', balanceChanges);
  ...
  if (print) console.log(user, 'decimalAdjustedBalanceChanges', decimalAdjustedBalanceChanges);
  ...
  const revenue = await calculateAbsDollarBalanceOnchain(
    unixtime,
    decimalAdjustedBalanceChanges,
    blockNumber,
    userTransfersInAndOut
  );
...
async function getCosts(unixtime: number, txHash: string): Promise<number | null> {
  try {
...
async function getRevenueClean(unixtime: number, event: any, market: MarketData): Promise<number | null> {
  const CALL_TRACE = await getCallTraceViaRpcProvider(event.transactionHash);

  const revenueBuyer = await getRevenueForAddress(unixtime, CALL_TRACE, market.llamma, market, event.blockNumber);
  if (revenueBuyer === null) return null;

  return revenueBuyer * -1;
}

export async function getSoftLiquidationRevenue(
  event: any,
  market: MarketData
): Promise<{ softLiquidationRevenue: number | null; timestamp: number | null }> {
  try {
    // const unixtime = Math.floor(new Date(event.dt).getTime() / 1000);
    const unixtime = await getBlockTimeStampFromNode(event.blockNumber);
    if (!unixtime) return { softLiquidationRevenue: null, timestamp: null };
...
export interface SoftLiquidationMeasurement {
  priceOracle: number;
  currentPrice: number;
  timestamp: number | null;
}

export async function meassureSoftLiquidation(
  event: any,
  market: MarketData
): Promise<SoftLiquidationMeasurement | null> {
  const unixtime = await getBlockTimeStampFromNode(event.blockNumber);
