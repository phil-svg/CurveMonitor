import { fetchLendingMarketsForChain, LendingMarketData } from './Utils.js';

async function runMarket(chain: string, market: LendingMarketData) {
...
async function runChain(chain: string) {
  ...
  const markets: LendingMarketData[] = marketsResponse.data;
