...
import { meassureSoftLiquidation } from './SoftLiquidationProfit.js';
import fs from 'fs';
import path from 'path';
import { getLlammaContract, getMintMarketControllerAbi, getMintMarketLlammaAbi, getMintMarketLlammaContract, } from './Contracts.js';
...
async function fetchSoftLiquidationEventsForMarket(market) {
...
async function processSingleSoftLiquidationEvent(event, market) {
    try {
        ///////////// NEW ///////////
        ...
        const softLiquidationEventInfo = {
            date: date,
...
async function getPAndOracleEveryBlock(market) {
...
async function getFullY(market, users, blockNumber, collatTokenDecimals) {
    ...
    for (let i = 0; i < users.length; i += batchSize) {
        ...
        const calls = chunk.map((user, index) => ({
        ...
        try {
            ...
            chunk.forEach((user, index) => {
                var _a, _b, _c;
                const result = (_c = (_b = (_a = results.results[`get_y_up-${index}`]) === null || _a === void 0 ? void 0 : _a.callsReturnContext[0]) === null || _b === void 0 ? void 0 : _b.returnValues) === null || _c === void 0 ? void 0 : _c[0];
                if (result) {
...
async function getAllMintMarketUserEverCurvePrices(market, blockNumber) {
...
async function filterForSL(market, addresses, blockNumber) {
    const multicall = new Multicall({ web3Instance: WEB3_HTTP_PROVIDER, tryAggregate: true });
    const abi = getMintMarketControllerAbi();
    const usersInSL = [];
    const batchSize = 500; // Adjust batch size as needed
    for (let i = 0; i < addresses.length; i += batchSize) {
        ...
        const calls = chunk.map((address, index) => ({
        ...
        try {
            ...
            chunk.forEach((address, index) => {
                var _a, _b;
                const userState = (_b = (_a = results.results[`userState-${index}`]) === null || _a === void 0 ? void 0 : _a.callsReturnContext[0]) === null || _b === void 0 ? void 0 : _b.returnValues;
...
async function getAllMintMarketUserInSL(market, blockNumber, collatTokenDecimals) {
    try {
...
async function processSingleMarket(market) {
    ...
    if (market.address.toLowerCase() !== '0xA920De414eA4Ab66b97dA1bFE9e6EcA7d4219635'.toLowerCase())
        return; // => WETH only
...
//# sourceMappingURL=CrvUSD.js.map