...
function removeFalsePositiveFlashLoans(detectedFlashLoans, readableTransfers) {
    ...
    return detectedFlashLoans.filter((flashLoan) => {
        ...
        for (const checkPosition of checkPositions) {
            const potentialBackTransfer = readableTransfers.find((transfer) => transfer.position === checkPosition &&
...
async function findFlashloan_s(readableTransfers) {
    let flashLoans = [];
    ...
    for (let i = 0; i < readableTransfers.length; i++) {
        ...
        if (currentTransfer.tokenSymbol === "ETH")
            continue; // Skip if the token is ETH
...
async function countFlashloans(txHashes) {
    let didFlashloanCount = 0;
    let didNotFlashloanCount = 0;
    let allFlashloanDetails = [];
...
function calculateFlashloanValues(flashloanDetails, tokenPrices) {
...
//# sourceMappingURL=FlashloanDetection.js.map