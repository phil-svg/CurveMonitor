...
function removeEthWrapsAndUnwraps(cleanedTransfers) {
    ...
    const filteredTransfers = cleanedTransfers.filter((transfer) => transfer.from.toLowerCase() !== WETH_ADDRESS.toLowerCase() && transfer.to.toLowerCase() !== WETH_ADDRESS.toLowerCase());
...
export async function getEnrichedCexDexDetails(enrichedTransaction) {
    ...
    const enrichedCexDexDetails = Object.assign(Object.assign({}, enrichedTransaction), { builder: builder, blockPayoutETH: blockPayoutETH, blockPayoutUSD: blockPayoutUSD, eoaNonce: eoaNonce, gasInGwei: gasInGwei, gasCostUSD: gasCostUSD, bribeInUSD: bribeInUSD });
