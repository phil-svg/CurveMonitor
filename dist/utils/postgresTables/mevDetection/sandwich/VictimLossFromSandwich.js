...
export async function calculateLossForSwap(parsedTx) {
    ...
    if (event === "TokenExchange")
        return await calculateLossForExchange(parsedTx);
    if (event === "TokenExchangeUnderlying")
        return await calculateLossForExchangeUnderlying(parsedTx);
    return null;
}
...
export async function calculateLossForExchangeUnderlying(parsedTx) {
    ...
    if (!roundedAmount || isNaN(Number(roundedAmount)))
        return null;
...
export async function calculateLossForDeposit(parsedTx) {
...
export async function calculateLossForWithdraw(parsedTx) {
    const outCoin = parsedTx.transactionCoins.find((coin) => coin.direction === "out");
    if (!outCoin)
        return null;
...
//# sourceMappingURL=VictimLossFromSandwich.js.map