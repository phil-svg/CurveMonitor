 ...
 */
export async function filterUncheckedTransactionIds(transactionIds) {
    try {
...
export async function fetchAtomicArbsForPoolAndTime(poolId, startUnixtime, endUnixtime) {
...
export async function printTopUniqueRevenueEntries(lowerBound, higherBound) {
    try {
        ...
        for (const entry of selectedEntries) {
            const txHash = await getTxHashByTxId(entry.tx_id);
            console.log(`Revenue: ${Math.round(entry.revenue)}, TxHash: ${txHash}`);
        }
    }
    catch (error) {
...
export async function getAtomicArbEntryByTxIdShort(txId) {
...
export async function getTxIdsWithAtomicArb() {
    const atomicArbs = await AtomicArbs.findAll({
        attributes: ["tx_id"],
        where: {
            is_atomic_arb: true,
