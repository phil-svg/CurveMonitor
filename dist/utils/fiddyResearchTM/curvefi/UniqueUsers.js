 ...
 */
async function getUniqueBuyersFromEventFor2024SwapsOnly() {
  ...
  `;
    ...
    const uniqueTraders = result.map((row) => row.trader);
 ...
 */
async function getUniqueFromAddressesForSwapsByRouter(routerAddress) {
    // Define the start and end of the year 2024 in UNIX timestamp format
    const startOfYear = new Date('2024-01-01T00:00:00Z').getTime() / 1000;
    const endOfYear = new Date('2024-12-31T23:59:59Z').getTime() / 1000;
  ...
  `;
    ...
    const uniqueFromAddresses = result.map((row) => row.from);
    console.log(`Number of unique 'from' addresses in swap transactions for 2024 linked to router ${routerAddress}: ${uniqueFromAddresses.length}`);
...
//# sourceMappingURL=UniqueUsers.js.map