...
import { saveEnhancedSwapDetailsToExcel } from '../utils/Excel.js';
...
export async function fetchSortedTransactionIdsByDateAndPool(poolAddress, startDate, endDate) {
...
async function findConsecutiveTransactionPairsWithMatchingSwaps(sortedTxIds) {
    const consecutiveTxPairs = [];
    const coinMovements = await findTransactionCoinsByTxIds(sortedTxIds);
    // Group coin movements by tx_id for easier comparison
    const movementsByTxId = {};
    ...
    for (let i = 0; i < sortedTxIds.length - 1; i++) {
        ...
        if (movementsByTxId[currentTxId] && movementsByTxId[nextTxId]) {
            ...
            const matchingOutCoins = currentOutCoins.every((coinId) => nextOutCoins.includes(coinId)) &&
...
async function getSwapDetailsAndImpactForTxPairs(matchingTxIds) {
    const result = [];
...
function groupSwapsByTokenPairs(swaps) {
    const groupedSwaps = {};
...
function groupEnhancedSwapsByTokenPairs(swaps) {
    const groupedSwaps = {};
    swaps.forEach((swap) => {
        const pairTokens = [swap.bought, swap.sold].sort(); // Sort the tokens alphabetically
...
function removeBoughtSoldProperties(groupedSwaps) {
    const modifiedSwaps = {};
...
function removeBoughtSoldPropertiesFromEnhanced(groupedSwaps) {
    const modifiedSwaps = {};
...
async function calculateSwapImpactIncludingTVL(poolAddress, startDate, endDate) {
    ...
    const enhancedSwapDetails = await Promise.all(priceImpactTxs.map(async (swapDetail) => {
        var _a;
        ...
        if (unixtime !== null) {
            for (const tvlChange of tvlChanges) {
                ...
                const balanceAtTx = ((_a = tvlChange.changes.find((change) => change.txId === swapDetail.txId)) === null || _a === void 0 ? void 0 : _a.newBalance) || tvlChange.startBalance;
        ...
        return Object.assign(Object.assign({}, swapDetail), { tvlPercentage,
            totalTvlUsdAtSwap });
    }));
    return enhancedSwapDetails;
}
export async function priceImpactThings() {
