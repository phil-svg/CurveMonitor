...
import { getCoinsBy, getPoolIdByPoolAddress, getNCoinsBy, getPoolsBySourceAddress, } from '../../postgresTables/readFunctions/Pools.js';
...
function filterTxTracesByPoolAddress(txTraces, poolAddress) {
...
async function checkIfTxWasCoinSymbolRemoval(txId, coinSymbol) {
...
async function checkIfTxWasCoinSymbolSupplyAdd(txId, coinSymbol) {
    const transactionCoin = await TransactionCoins.findOne({
        include: [
            {
                model: Coins,
...
async function checkIfTxHasMultipleEntries(txId) {
...
export async function getGasUsedArrayForPoolAndFunction(poolAddress, functionName, txHash) {
    ...
    if (!methodIdsForPoolAddressLight)
        return [];
...
async function getAllTxHashesForPoolAndTimeWindow(poolAddress, startDate, endDate) {
...
function getGasUsageBracket(gasUsed) {
...
function sortGasUsageBrackets(gasUsageBrackets) {
    const sortedGasUsageBrackets = {};
...
function convertCountsToPercentages(sortedGasUsageBrackets) {
...
async function getGasUsageBrackets(stableswapPoolAddressArr, startDate, endDate) {
    let gasUsageBrackets = {};
    ...
    let gasUsageChecks = 0, txHashesChecks = 0, poolChecks = 0, numOfCounts = 0, poolCounter = 0;
...
async function getGasUsageBracketsForSpecificConditions(stableswapPoolAddressArr, startDate, endDate) {
    let gasUsageBrackets = {};
    const relevantFunctionNames = ['add_liquidity'];
    const targetCoinSymbol = 'WETH';
    const N_COIN_CONDITION = 2;
    let gasUsageChecks = 0, txHashesChecks = 0, poolChecks = 0, numResults = 0, poolCounter = 0;
    poolChecks += stableswapPoolAddressArr.length;
    for (const poolAddress of stableswapPoolAddressArr) {
        const N_COINS = await getNCoinsBy({ address: poolAddress });
        if (N_COINS !== N_COIN_CONDITION)
            continue;
        ...
        if (!poolCoins.some((coinAddress) => coinAddress.toLowerCase() === targetCoinAddress.toLowerCase()))
            continue;
        poolCounter++;
        const allTxHashes = await getAllTxHashesForPoolAndTimeWindow(poolAddress, startDate, endDate);
        if (!allTxHashes)
...
export async function gasUsageThings() {
 ...
 */
async function loadSandwichDataFromCSVfile() {
    return new Promise((resolve, reject) => {
        const sandwichData = [];
        const parser = createReadStream('../sandwiches-ng.csv').pipe(parse({
            columns: true,
