import { addressIsWhitelisted, isLeafAddress, removeEthWrapsAndUnwraps, } from '../../../../../postgresTables/mevDetection/cexdex/utils/cexdexDetection.js';
...
async function wasPoolContractCalledDirectly(poolContractAddress, to) {
...
export async function isCexDexArbCandidateWOdb(from, to, txDetails, numOfTransfers, cleanedTransfers) {
...
export async function isCexDexArbWOdb(cleanedTransfers, from, to, blockNumber, txPosition, txDetails, poolContractAddress, web3HttpProvider, chain) {
    ...
    if (addressIsWhitelisted(to))
        return false;
    if (await wasPoolContractCalledDirectly(poolContractAddress, to))
        return false;
    if (!(await isCexDexArbCandidateWOdb(from, to, txDetails, numOfTransfers, cleanedTransfers)))
        return false;
