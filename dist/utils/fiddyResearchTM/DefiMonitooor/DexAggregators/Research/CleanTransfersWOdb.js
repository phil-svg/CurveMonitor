...
export async function parseEventsFromReceiptForEntireTxWOdb(txHash, chain) {
...
export async function getTokenDetailsForTokenArrayWOdb(uniqueTokenArray, web3HttpProvider) {
...
export async function makeTransfersReadableWOdb(tokenTransfers, web3HttpProvider) {
    ...
    const tokenDetailsArray = await getTokenDetailsForTokenArrayWOdb(uniqueTokenArray, web3HttpProvider);
...
export async function getCleanedTransfersWOdbWOtraceForChain(txHash, chain, web3HttpProvider, tx) {
    const parsedEventsFromReceipt = await parseEventsFromReceiptForEntireTxWOdb(txHash, chain);
    ...
    const readableTransfers = await makeTransfersReadableWOdb(transfers, web3HttpProvider);
    let updatedReadableTransfers = [];
    if (tx) {
        updatedReadableTransfers = updateTransferList(readableTransfers, tx.to);
    }
