...
export async function getUniswapV3RequiredSwapAmountForTargetPriceWithAddedValue(poolAddress, targetPrice, blockNumber) {
    ...
    console.log('sPriceCurrent', sPriceCurrent, '=', (10 ** (token0decimals - token1decimals) * sPriceCurrent ** 2) / 2 ** 192 // this prints the price used in the examples
    );
    ...
    if (sPriceTarget > sPriceCurrent) {
        while (sPriceTarget > sPriceCurrent) {
            if (sPriceTarget > sPriceUpper) {
                ...
                if (Number(nextTickRange.liquidityNet) === 0)
                    emptyTicksCounter++;
            ...
            if (emptyTicksCounter === 3)
                return "can't reach price, not enough liquidity";
        }
        ...
        while (sPriceTarget < sPriceCurrent) {
            if (sPriceTarget < sPriceLower) {
                ...
                if (Number(currentTickRange.liquidityNet) === 0)
                    emptyTicksCounter++;
    ...
    console.log('\nfinal results:');
    console.log('sPriceCurrent', sPriceCurrent, '=', (10 ** (token0decimals - token1decimals) * sPriceCurrent ** 2) / 2 ** 192);
