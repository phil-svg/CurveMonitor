 ...
 */
export async function getFlexibleSymbolForTradingPair(asset1, asset2) {
    try {
        const response = await fetch(`${API_URL}/api/v3/exchangeInfo`);
        if (!response.ok) {
 ...
 */
export function findMatchingTrade(trades, targetTimestamp, targetVolume, isOnChainBuy) {
    const matchesCriteria = (trade, timeDiff, volumePercentage) => {
